...
export async function POST(request: Request) {
  ...
  if (!user?.username || !user.passwordHash || !body.password || !verifyPassword(body.password, user.passwordHash)) return Response.json({ error: "Invalid username or password." }, { status: 401 });
