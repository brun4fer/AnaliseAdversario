...
import { ensureMediaWorkspace, type MediaLocalAccount } from "@/lib/media-workspace";

function tokenHash(token: string) {
  return createHash("sha256").update(token.trim()).digest("hex");
}

export async function getMediaLinkStatus(account: MediaLocalAccount) {
...
export async function createMediaLinkToken(account: MediaLocalAccount) {
  const { mediaWorkspace } = await ensureMediaWorkspace(account);
...
export async function claimMediaLinkToken(account: MediaLocalAccount, token: string) {
