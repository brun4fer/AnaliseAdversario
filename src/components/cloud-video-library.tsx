...
import { Button, FieldLabel, Panel } from "@/components/ui";
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <Panel className="flex max-h-[80dvh] w-full max-w-2xl flex-col overflow-hidden">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 p-4">
          <div>
            <div className="flex items-center gap-2 text-cyan-200"><FolderOpen size={18} /><FieldLabel>Shared cloud library</FieldLabel></div>
            <h2 className="mt-2 text-lg font-bold text-white">Choose an existing video</h2>
            <p className="mt-1 text-xs text-slate-400">Only videos belonging to this account are shown. Selecting one does not upload another copy.</p>
          </div>
        ...
        <div className="min-h-40 flex-1 overflow-y-auto p-3">
          {loading ? (
            ...
            <div className="flex h-40 flex-col items-center justify-center text-center"><Cloud size={30} className="text-slate-600" /><p className="mt-3 text-sm font-medium text-slate-300">No shared videos yet</p><p className="mt-1 text-xs text-slate-500">Use Upload new to add the first video to this account.</p></div>
          )}
        </div>
      </Panel>
    </div>
