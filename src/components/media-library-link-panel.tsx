    ...
    <Panel className="overflow-hidden">
      <div className="flex items-start justify-between gap-4 border-b border-white/10 p-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-slate-500">Link this account to the same client in the Player and Team applications. Videos remain private and are never matched using the username alone.</p>
        </div>
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="rounded-lg border border-white/10 bg-white/[.025] p-3">
            ...
            <p className="mt-1 text-xs text-slate-500">Paste a code created in another application for this same client. The code expires after 30 minutes and can only be used once; the link itself does not expire.</p>
          ...
          <div className="rounded-lg border border-white/10 bg-white/[.025] p-3">
            ...
            <p className="mt-1 text-xs text-slate-500">Create a temporary code, then paste it into the other application while signed in to the same client account.</p>
            {generated ? <div className="mt-3 flex gap-2"><TextInput readOnly value={generated.token} className="font-mono text-xs" /><Button onClick={() => void copyCode()}><Copy size={14} />Copy</Button></div> : <Button className="mt-3" disabled={working} onClick={() => void createCode()}>{working ? <Loader2 size={14} className="animate-spin" /> : <Link2 size={14} />}Create linking code</Button>}
          </div>
        </div>
