...
export function SubmomentEditor({ matchId, perspective }: { matchId: string; perspective: AnalysisPerspective }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    (videoStorageStatus === "READY" ? getRemoteVideoUrl(matchId, "analysis").catch(() => null) : Promise.resolve(null))
      .then(async (remote) => {
        if (!active) return;
  ...
  function seekTo(seconds: number) { const video = videoRef.current; if (!video || !selectedMoment) return; const next = Math.max(selectedMoment.startTimeSeconds, Math.min(selectedMoment.endTimeSeconds, seconds)); video.currentTime = next; setCurrentTime(next); }
